...
##### expo-sqlite/kv-store (React Native)

```js
import { configureObservablePersistence } from '@legendapp/state/persist';
import { ObservablePersistSQLiteStorage } from '@legendapp/state/persist-plugins/expo-sqlite';

// Global configuration
configureObservablePersistence({
    // Use SQLiteStorage in React Native
    pluginLocal: ObservablePersistSQLiteStorage,
    localOptions: {
        asyncStorage: {
            // The SQLiteStorage plugin needs to be given the implementation of SQLiteStorage
            SQLiteStorage,
        },
    },
});
...
If you are on React Native you will need to install `react-native-mmkv` or `@react-native-async-storage/async-storage` or `expo-sqlite`, depending on which one you choose to use.
...
const obs$ = persistObservable(
    { initialValue: 'hello' },
    {
        pluginRemote: {
            get: ({ onChange }) => {
                const getFn = () => fetch('https://url.to.get').then((res) => res.json());
    ...
    { initialValue: 'hello' },
    {
        pluginRemote: persistPluginQuery({
            ...
            query: {
                ...
                queryFn: () => {
                    return fetch('https://url.to.get?page=' + page$.get()).then((res) => res.json());
                },
            },
        }),
    }
