...
Because [observing contexts](../reactivity/#observing-contexts) track all observable `get()` calls, any functions you call within an observer will also be reactive.
...
Any function in an observable can be used a computed observable, whether at the root or in any child. A function is turned into an observable when you first call `get()` on it. It will then re-compute itself whenever the observables it accesses with `get()` are changed.
...
The difference between using it as a function vs. as a computed observable is that a computed observable is an object that caches the value. So `fullName()` will re-compute whenever you call it, but `fullName.get()` creates a computed observable that re-computes itself whenever its dependencies change.
...
You can access the status of an async observable with the [syncState](usage/cache-sync/#syncstate) helper, which is an observable itself. The most common usage is to check its loaded or error states:
