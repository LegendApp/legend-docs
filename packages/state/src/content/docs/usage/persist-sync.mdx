...
const store$ = observable({
    users: synced({
        ...
        get: () => fetch('https://reqres.in/api/users')
                  .then(r => r.json())
                  .then(r => r.data),
...
`get()` is an observing context, so if you get an observable's value it will re-run if it changes. We can use that to created a paging query by setting the query mode to "append" (or "assign" if it's a record) to append new pages into the observable array.
...
const store$ = observable({
    usersPage: 1,
    users: synced({
        get: () => fetch(`https://reqres.in/api/users?page=${store$.usersPage.get()}`)
                  .then(r => r.json())
                  .then(r => r.data),
        mode: 'append'
    }),
