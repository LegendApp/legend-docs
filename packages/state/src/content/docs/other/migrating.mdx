...
Version 3.0 is a big change focused mainly in four areas:
...
Likewise, `configureObservablePersistence` is renamed to `configureObservableSync`.

```ts
import { configureObservableSync, syncObservable, syncState } from "@legendapp/state/sync"
// Change
...
It's now much easier to sync individual observables with a new plugin system. The Firebase plugin is still in progress, so if you were using that you'll want to hold off on the upgrade until the full 3.0 release.

See the [Persist and Sync docs](../../sync/persist-sync/) for the latest.

If you had made a custom sync plugin on the old system, please post an issue on [Github](https://github.com/LegendApp/legend-state) or talk to us on [Slack](https://join.slack.com/t/legendappcommunity/shared_invite/zt-1mfjknpna-vUL2s1qOuNeZL12~t2RruQ), and we will help you with the migration.
