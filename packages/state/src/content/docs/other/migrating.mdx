...
To migrate you can remove usage of `enableLegendStateReact()` or `enableReactDirectRender` as well as any usage of direct rendering, and replace it with `Memo`. When you remove those it will stop overriding the types so rendering observables directly will result in TypeScript errors.

If you were using `enableLegendStateReact` to render direct observables heavily, [enableReactDirectRender](../../usage/configuring#enablereactdirectrender-deprecated) is still usable, though it will cause TypeScript errors to help you migrate away.
...
#### persistObservable returns an object with { state }
