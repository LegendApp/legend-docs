...
`usePersistedObservable` returns an observable like [useObservable](../react-api#useobservable) with options to automatically persist it, the same as [persistObservable](../../guides/persistence).
