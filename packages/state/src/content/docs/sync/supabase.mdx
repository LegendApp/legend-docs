...
3. `first`: Return the first result of the query and treat it as a single object
...
Note that it's not possible to catch deleted rows with this method, so you will have to use [soft deletes](#soft-deletes) described in the next session. If you don't need to delete you can remove that column from the script.
...
The delete parameter does not need to be an actual `delete` action in Supabase. You could also implement it as a soft delete if you prefer, just setting a `deleted` field to true. To change the plugin to set that field instead of deleting the row, just set the `fieldDeleted` option to the column name.
