...
The sync features are designed to be used through a plugin for your backend of choice. The plugins are all built on top of [synced](TODO) and are configurable with their own options as well as general sync and persist options.

### SQL plugins

- [Keel](TODO): The backend we use for Bravely
- [Supabase](TODO): Popular PostgreSQL backend

These are built on top of the CRUD plugin.

### General

- [CRUD](TODO): Supports any backend with list, get, create, update, delete actions
- [Fetch](TODO): A wrapper around fetch to reduce boilerplate
...
This page will show how you use the core [synced](TODO). The plugins are built on top of `synced` so everything on this page applies to the plugins as well.
...
First set the general sync and persist options, then configure your sync plugin with its specific options. In the [Keel](../keel) and [Supabase](../supabase) plugins you can configure these in their global configuration, or you can set them in their `synced` variant.
