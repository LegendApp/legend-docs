...
const state$ = observable(
    synced({
        ...
        set: ({ value }) => fetch('https://url.to.set', { method: 'POST', data: JSON.stringify(value) }),
        persist: {
            name: 'test',
        },
    }),
...
const state$ = observable(
    synced({
        ...
        transform: combineTransforms(transformStringifyDates(), transformStringifyKeys('jsonData', 'messagesArr'), {
            load: async (value) => {
...
const persistOptions = configureSynced({
    persist: {
        plugin: observablePersistSQLiteStorage({
...
If you're persisting a large amount of data you may want to show a loading state while persistence is loading.
