...
- [Keel](../keel): The backend we use for Bravely
...
const mySyncPlugin = configureSynced(syncedCrud, {
    persist: {
...
const profile$ = observable(mySyncPlugin({
    list: () => {/*...*/},
    create: () => {/*...*/},
    update: () => {/*...*/},
    delete: () => {/*...*/},
...
Sync plugins have a lot of options so you'll likely want to set some defaults. You can do that with the `configureSynced` function to create a customized version of a plugin with your defaults. You will most likely want to at least set a default persistence plugin.
...
const myPersistOptions = configureSynced({
    persist: {
        plugin: ObservablePersistMMKV
    }
})

// Or create your own plugin derived from synced with defaulted options
const mySyncPlugin = configureSynced(synced, {
    persist: {
        plugin: ObservablePersistMMKV
    }
})
...
```js
const status$ = syncObservable(state$, persistOptions({
    persist: {
        name: 'store' // IndexedDB table name
    }
}))
