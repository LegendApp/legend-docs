...
You can create an observable with a function at the root or in any child. The computed function is activated when you call `get()` on it.
...
// Or create a fully computed observable
const name$ = observable(() => state$.fname.get() + ' ' + state$.lname.get());

// Accessing a computed activates it
