...
React components can track access to `get()` on observables and automatically re-render whenever they change.
...
Legend-State includes a powerful [sync and persistence system](../../guides/persistence). It easily enables local-first apps by optimistically applying all changes locally first, ensuring they eventually sync, managing conflict resolution, and syncing minimal diffs. We use Legend-State as the sync systems in [Legend](https://legendapp.com) and [Bravely](https://bravely.io), so it is by necessity very full featured while being simple to set up.
