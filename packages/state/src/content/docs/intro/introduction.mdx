...
:::caution
Version 3 is in Beta. You can use it by installing the @beta version but keep in mind that the APIs may change slightly before release. If you're starting a new project we suggest starting with version 3 since it's much improved over version 2.
...
Legend-State handles local, global, and remote state all in one. Just `get()` and `set()` observables, and a robust sync system makes sure your state is persisted locally and synced with your server.

##### 2. Great DX and less code

Just `get()` in an `observer` to automatically re-render whenever state changes, and `set()` to update it. With the reduced boilerplate and everything that Legend-State does for you, you'll build better apps with less code.
...
Legend-State helps your re-renders smaller and less frequent, making your apps faster 🔥.

<Primitives />

### 4. 💾 Powerful sync and persistence

Legend-State includes a powerful [sync and persistence system](../../sync/persist-sync). It easily enables local first apps by optimistically applying all changes locally first, retrying changes even after restart until they eventually sync, and syncing minimal diffs. We use Legend-State as the sync systems in [Legend](https://legendapp.com) and [Bravely](https://bravely.io), so it is by necessity very full featured while being simple to set up.
...
Version 3 is currently available in the @beta version and may change before the final release.
