...
export function SectionBadges() {
    ...
    return (
        <div className="text-center pt-16 !mt-0">
            <Header size="h2">All in One</Header>
            <Text className="max-w-lg mx-auto text-left">
                Legend-State is the fastest React state library, and it takes care of all of the hard sync and caching
                stuff for you.
            </Text>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:flex justify-center border bg-tBg border-tBorder rounded-lg shadow-tShadowDark backdrop-blur-sm bg-opacity-0">
                {badges.map(({ title, subtitle }, i) => (
                    ...
                    >
                        ...
                        {subtitle && <div className="text-sm sm:text-md text-white/60 pt-1 !mt-0">{subtitle}</div>}
                    </div>
                ))}
            </div>
        </div>
