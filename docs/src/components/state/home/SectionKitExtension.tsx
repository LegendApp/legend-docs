...
export const SectionKitExtension = observer(function SectionKitExtension() {
    ...
    return (
        <div className="mt-subsection px-4">
            ...
            <Text>A contextually aware coding assistant to accelerate your development speed.</Text>
            <div className="lg:flex items-center gap-4 pt-8">
                <FeatureGrid />
                <motion.div
    ...
    return (
        <div className="grid grid-cols-2 gap-4 flex-1">
            {features.map((feature, index) => (
                <div key={index} className="p-4 border rounded-lg border-tBorder shadow-tShadowDark bg-tBg !mt-0">
                    <Header size="h5" className="font-bold mb-2">
                        {feature.title}
                    </Header>
                    <Text className="text-sm text-zinc-300">{feature.description}</Text>
                </div>
            ))}
        </div>
    );
