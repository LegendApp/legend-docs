...
description: Why React feels slow by default, and a reactive model that makes apps faster and easier to build.
...
1. <span className="mr-2">⚡️</span> Much faster
2. <span className="text-xl mr-2">🦄</span> Much easier to use
</div>
...
<div className="text-xl font-bold text-zinc-100">
1. <span className="text-xl mr-2">🐢</span> It's slow by default
2. <span className="text-xl mr-2">🤯</span> Hooks are too complex
</div>
...
An enormous amount of the React experience is compensating for the fact that components are constantly re-rendering, and those complex workarounds are the source of much of the mental overhead 🤯.
...
`}
  noInline={true}
  renderCode=";render(<Box className='flex flex-col gap-y-3'><HooksName /><LegendStateName /></Box>)"
  transformExamples={[
    {
      pattern: '<input value={name} onChange={onInputChange} />',
      replacement: '<input className="border border-zinc-400 rounded-lg px-3 py-2" value={name} onChange={onInputChange} />'
    },
    {
      pattern: '<$React.input $value={profile$.name} />',
      replacement: '<$React.input className="border border-zinc-400 rounded-lg px-3 py-2" $value={profile$.name} />'
    }
  ]}
/>
...
function HooksTreeRight() {
    return <Box>Unrelated element</Box>
}
function HooksTree() {
    const [count, setCount] = useState(1)
...
function LegendTreeLeaf({ $count }) {
    return <Box>Count: <Memo>{$count}</Memo></Box>
}
...
function HooksTreeLeaf({ count }) {
    const renderCount = ++useRef(-1).current;
    return (
        <FlashingDiv pad className="border border-zinc-700 text-sm w-full">
            <div className="font-bold text-xs">Leaf</div>
            <div className="pt-1 text-[11px] text-fd-muted-foreground">Renders: {Math.max(renderCount, 1)}</div>
            <div className="pt-2">Count: {count}</div>
        </FlashingDiv>
    );
}
...
function HooksTreeLeft({ count }) {
    const renderCount = ++useRef(-1).current;
    return (
        <FlashingDiv pad bg="bg-fd-card" className="border border-fd-border text-sm w-full">
            <div className="font-bold text-xs">Intermediate</div>
            <div className="pt-1 text-[11px] text-fd-muted-foreground">Renders: {Math.max(renderCount, 1)}</div>
            <div className="pt-3">
                <HooksTreeLeaf count={count} />
            </div>
        </FlashingDiv>
    );
}
...
function HooksTreeRight() {
    ...
    return (
        <FlashingDiv pad bg="bg-fd-card" className="border border-fd-border text-sm w-full">
            <div className="font-bold text-xs">Unrelated element</div>
            <div className="pt-1 text-[11px] text-fd-muted-foreground">Renders: {Math.max(renderCount, 1)}</div>
        </FlashingDiv>
    );
}
...
function HooksTree() {
    ...
    return (
        <FlashingDiv pad className="border border-zinc-700 rounded-lg w-80 text-sm">
            <div className="font-bold">Normal React tree</div>
            <div className="pt-1 text-[11px] text-fd-muted-foreground">Renders: {Math.max(renderCount, 1)}</div>
            <div className="pt-2">Count: {count}</div>
    ...
    return (
        <FlashingDiv pad bg="bg-fd-card" className="border border-fd-border text-sm w-full">
            ...
            <div className="pt-2 leading-loose">
                ...
                <Memo>
                    {() => (
                        <FlashingDiv span className="inline-flex rounded bg-fd-secondary px-2 py-1">
                            {$count.get()}
                        </FlashingDiv>
                    )}
                </Memo>
...
function LegendTree() {
    ...
    return (
        <FlashingDiv pad bg="bg-fd-card" className="border border-fd-border rounded-lg w-80 text-sm">
            <div className="font-bold">Legend-State tree</div>
            <div className="pt-1 text-[11px] text-fd-muted-foreground">Renders: {Math.max(renderCount, 1)}</div>
            <div className="pt-2 leading-loose">
                Count:{' '}
                <Memo>
...
`}
  noInline={true}
  renderCode=";render(<Box className='flex flex-col gap-y-3'><HooksChat /><div className='mt-4' /><LegendStateChat /></Box>)"
/>
