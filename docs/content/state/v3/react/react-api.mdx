...
const Component = ({ id }) => {
    ...
    return (
        <div>
            <div>Selected: {isSelected ? 'Yes' : 'No'}</div>
            <div>Theme: {theme}</div>
            <button onClick={() => state$.selected.set(id)}>
                Select me
            </button>
            <button onClick={() => state$.theme.set(theme === 'dark' ? 'light' : 'dark')}>
                Toggle theme
            </button>
        </div>
    )
  ...
  noInline={true}
  renderCode=";render(<Component id={1} />)"
  previewWidth={280}
/>
