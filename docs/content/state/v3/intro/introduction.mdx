...
// Create observables for different primitive types
const count$ = observable(0)
const name$ = observable('Legend')
const isVisible$ = observable(true)

function PrimitivesExample() {
  const count = use$(count$)
  const name = use$(name$)
  const isVisible = use$(isVisible$)

  return (
    <div style={{ padding: '16px', fontFamily: 'system-ui' }}>
      <h3>Fine-grained reactivity with primitives</h3>
      ...
      <div style={{ marginBottom: '16px' }}>
        <label>
          <input
            type="checkbox"
            checked={isVisible}
            onChange={e => isVisible$.set(e.target.checked)}
            style={{ marginRight: '8px' }}
          />
          Show message
        </label>
  ...
  noInline={true}
  renderCode=";render(<PrimitivesExample />)"
  previewWidth={300}
/>
