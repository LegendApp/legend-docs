...
<Callout title="v2 Beta">
Version 2 is in beta and is much faster/better, so the docs encourage using the beta. But if you feel more comfortable with a stable version, just remove the @next from the install.
</Callout>
