...
An `optimized` prop adds additional optimizations, but in an unusual way by re-using React nodes. See [Optimized rendering](../../4-guides/2-performance#optimized-rendering) for more details.
...
In this example, `reactive` adds a `$message` prop which takes a [Selector](../../1-introduction/2-getting-started#selectors), while the target component receives a normal `message` prop and is only re-rendered when `message` changes.
