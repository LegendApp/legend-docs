...
`useObserve` creates an [observe](../../2-usage/2-reactivity#observe) which you can use to take actions when observables change. This can be effectively similar to `useEffect` for observables, except that it runs when observables change and not because of a deps array changing.
...
Its observables will not be automatically tracked for re-rendering, so you can track them [the same as any other observable](../1-react-introduction#use-observable-state).
...
`useComputed` is like `useObservable` and creates a [computed](../../2-usage/2-reactivity#computed) observable.
