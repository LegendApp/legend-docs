...
Legend-State is the [fastest React state library](../3-fast-), designed to be as efficient as possible. It does very little extra work and minimizes renders by only re-rendering components when their observables change. And at only `4kb` it won't hurt your bundle size.
...
Using features like [Memo](../../3-react/3-fine-grained-reactivity#Memo) it's easy to isolate renders to the smallest possible change.

<Primitives />

For isolating a group of elements or computations, Legend-State has [built-in helpers](../../3-react/3-fine-grained-reactivity) to easily extract children so that their changes do not affect the parent. This keeps large parent components from rendering often just because their children change.
...
Legend-State does everything above-board using hooks, with all React functionality built on top of [useSelector](../../3-react/2-react-api#useselector), which just uses `useSyncExternalStore`. Check [the source](https://github.com/LegendApp/legend-state/blob/main/src/react/useSelector.ts) to see the lack of hackery.
...
See [Patterns](../../4-guides/3-patterns) for more examples of different ways to use Legend-State.

## Persistence built in 💾

> There are only two hard things in Computer Science: cache invalidation and naming things. - Phil Karlton

We don't want developers to have to worry about persisting and syncing state, because it's often very complicated and error-prone. So we've built [persistence](../../4-guides/1-persistence) plugins using Legend-State's listeners, with extensive tests to make sure it's absolutely correct.
...
Observables prevent direct assignment, favoring more purposeful `set` and `assign` functions instead. Read more in [safety](../../2-usage/1-observable#safety).
