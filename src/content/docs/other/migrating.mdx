...
The direct rendering enabled by `enableLegendStateReact` and `enableReactDirectRender` was fragile, hard to find in files, and the React team advised against it. So instead we are using the `Memo` component. See [Render an observable directly](../../3-react/3-fine-grained-reactivity/#render-an-observableselector-directly) for more details.
...
The reactive components are now better named and more easily customizable with configuration functions, exported from the normal `/react` path. See [Reactive components](../../3-react/3-fine-grained-reactivity/#reactive-components) for more details.
...
See [persistObservable](../../4-guides/1-persistence#persistobservable) for details.
...
Since there were not any remote persistence plugins before, these changes would likely not affect you unless you made your own. See [Remote Persistence](../../4-guides/1-persistence#remote-persistence) for details.
...
It was actually slower in our testing so we simplified things and just removed it. See [IndexedDB](../../4-guides/1-persistence#IndexedDB) for up-to-date docs.
...
See [the React guide](../../3-react/1-react-introduction) for how we suggest setting up your components now.
...
See [reactive props](../../3-react/3-fine-grained-reactivity) for more detauls.
...
`ref` was a bit unclear and conflicted with React - the new feature to [directly render observables](../../3-react/3-fine-grained-reactivity/#render-an-observable-directly) requires a `ref` property. So it is now renamed to `obs`, which feels more intuitive as it is used to get an observable.
